...
#
# Banafshe Bamdad, Sa Jun 17, 2023 22:13:15
#
# This script runs the required commands in parallel in order to record ros bag file
# Usage: ./collect_data_ZHAW.bash bag_file_name
#        e.g. $ ./collect_data_ZHAW.bash my_test 
#

# check if the bag_file_name is passed to the script
if [ -z "$1" ]; then
  echo "Usage: ./collect_data_ZHAW.bash.bash bag_file_name"
  exit 1
fi
...
roslaunch realsense2_camera banafshe_ZHAW_D435i_rs_camera.launch &
...
rosbag record -O "$file_name"_ZHAW_D435i.bag /camera/accel/imu_info  /camera/gyro/imu_info /camera/imu /camera/infra1/camera_info /camera/infra1/image_rect_raw /camera/infra2/camera_info /camera/infra2/image_rect_raw /camera/color/image_raw /camera/depth/image_rect_raw /camera/color/camera_info
